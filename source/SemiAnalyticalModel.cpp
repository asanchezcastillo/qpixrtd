...
{ 
  ...
  double cosine;
  cosine = std::abs(relative.z) / distance; // Z is the drift direction
 ...
 if(fDetectorVDConfiguration) 
 {
  solid_angle = Rectangle_SolidAngleVD(Dims{opDet.h, opDet.w}, abs_relative); 
 }

 else
 {
  solid_angle = Rectangle_SolidAngleHD(Dims{opDet.h, opDet.w}, abs_relative);
 }
 ...
 // accounting for solid angle and LAr absorbtion length
  ...
  double r = std::hypot(ScintPoint.x - fanode_centre.x, ScintPoint.y - fanode_centre.y);
...
//................... 
//Solid angle for detector in the plane XZ (VD geometry)
double SemiAnalyticalModel::Rectangle_SolidAngleVD(SemiAnalyticalModel::Dims const& out, SemiAnalyticalModel::Vector_t const& v) const {
   
  if( v.y==0.0 && v.x==0.0){
    return Rectangle_SolidAngle(out.w,out.h,v.z);
  }

  if( (std::abs(v.y) > out.w/2.0) && (std::abs(v.x) > out.h/2.0)){
    double A, B, a, b, d;
    A = std::abs(v.y)-out.w/2.0;
    B = std::abs(v.x)-out.h/2.0;
    a = out.w;
    b = out.h;
    d = std::abs(v.z);
...
double SemiAnalyticalModel::Rectangle_SolidAngleHD(SemiAnalyticalModel::Dims const& out, SemiAnalyticalModel::Vector_t const& v) const {
...
void SemiAnalyticalModel::CheckOverlaps() const
{
  ...
  if(!fDetectorVDConfiguration)
  {
   for(size_t i = 0; i<fOpDetCenter.size(); i++)
   {
    ...
    {
      if( ( (fOpDetCenter.at(i).x-(fOpDetHeight.at(i)/2))<(fOpDetCenter.at(j).x+(fOpDetHeight.at(j)/2)) 
      && (fOpDetCenter.at(j).x-(fOpDetHeight.at(j)/2))<(fOpDetCenter.at(i).x+(fOpDetHeight.at(i)/2))) &&
